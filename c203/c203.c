...
void Queue_Init( Queue *queue ) {
    //kontrola existence queue
    if(queue == NULL) Queue_Error(QERR_INIT);
    // projedu index po indexu a nastavim *
    for (int i = 0; i < QUEUE_SIZE; ++i) {
        queue->array[i] = '*';
    }
    //nastavim index viz zadani
    queue->firstIndex = 0;
    queue->freeIndex = 0;
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
    // gcc warn (queue == NULL || Queue_IsEmpty(queue)) ? Queue_Error(QERR_FRONT) : (*dataPtr = queue->array[queue->firstIndex]);
...
void Queue_Remove( Queue *queue ) {
    //gcc warn	(queue == NULL || Queue_IsEmpty(queue)) ? (Queue_Error(QERR_REMOVE)) : (queue->firstIndex = nextIndex(queue->freeIndex));
	if(queue == NULL || Queue_IsEmpty(queue)){
